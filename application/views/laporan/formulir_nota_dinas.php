...
foreach ($get_setSkpdWhereKd as $gsswk) {
    ...
    $email = $gsswk->email;
    $kouta_anggaran_dalam = $gsswk->kouta_anggaran_dalam;
    $kouta_anggaran_luar = $gsswk->kouta_anggaran_luar;
}
            ...
            case "persetujuan":
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <table class="table">
                            <?php
                            foreach ($get_realisasiAnggaran as $row_rea) {
                                if ($row_rea->kode_skpd == $id_skpd) {
                                    $kouta_dlm = $row_rea->kouta_anggaran_dalam;
                                    $kouta_luar = $row_rea->kouta_anggaran_luar;
                                    $jml_dlm = $row_rea->jml_realisasi_dalam;
                                    $jml_luar = $row_rea->jml_realisasi_luar;

                                    $sisa_dlm = $kouta_dlm - $jml_dlm;
                                    $sisa_luar = $kouta_luar - $jml_luar;
                                }
                            }
                            if ($id_ref_tujuan == 1 or $id_ref_tujuan == 2 or $id_ref_tujuan == 3) {
                                foreach ($get_realisasiAnggaranLuarWhereSkpd as $rea_lr) {
                                    if ($no_nd == $rea_lr->no_nota_dinas) {
                                        $perkiraan_luar = $rea_lr->realisasi_luar;
                                    }
                                }
                            } else {
                                foreach ($get_realisasiAnggaranDalamWhereSkpd as $rea_dlm) {
                                    if ($no_nd == $rea_dlm->no_nota_dinas) {
                                        $perkiraan_dlm = $rea_dlm->realisasi_dalam;
                                    }
                                }
                            }
                            $perkiraan_biaya_luar = $sisa_luar + $perkiraan_luar;
                            $perkiraan_biaya_dlm = $sisa_dlm + $perkiraan_dlm;
                            ?>
                            <tr>
                                <td width="40%">Pagu Anggaran Tahun 2018</td>
                                <td width="3%">:</td>
                                <td>Rp. 
                                    <?php
                                    if ($id_ref_tujuan == 1 or $id_ref_tujuan == 2 or $id_ref_tujuan == 3) {
                                        echo number_format($kouta_anggaran_luar, 2, ',', '.');
                                    } else {
                                        echo number_format($kouta_anggaran_dalam, 2, ',', '.');
                                    }
                                    ?>
                                </td>
                            </tr>
                            <tr>
                                <td width="40%">Realisasi Sampai Saat ini</td>
                                <td width="3%">:</td>
                                <td>Rp. 
                                    <?php
                                    $realisasi_luar = $kouta_anggaran_luar - $perkiraan_biaya_luar;
                                    $realisasi_dalam = $kouta_anggaran_dalam - $perkiraan_biaya_dlm;
                                    if ($id_ref_tujuan == 1 or $id_ref_tujuan == 2 or $id_ref_tujuan == 3) {
                                        echo number_format($realisasi_luar, 2, ',', '.');
                                    } else {
                                        echo number_format($realisasi_dalam, 2, ',', '.');
                                    }
                                    ?>
                                </td>
                            </tr>
                            <tr>
                                <td width="40%">Sisa Anggaran</td>
                                <td width="3%">:</td>
                                <td>Rp. 
                                    <?php
                                    if ($id_ref_tujuan == 1 or $id_ref_tujuan == 2 or $id_ref_tujuan == 3) {
                                        echo number_format($perkiraan_biaya_luar, 2, ',', '.');
                                    } else {
                                        echo number_format($perkiraan_biaya_dlm, 2, ',', '.');
                                    }
                                    ?>
                                </td>
                            </tr>
                            <tr>
                                <td>Perkiraan Anggaran Keberangkatan</td>
                                <td>:</td>
                                <td>Rp.
                                    <?php
                                    if ($id_ref_tujuan == 1 or $id_ref_tujuan == 2 or $id_ref_tujuan == 3) {
                                        echo number_format($perkiraan_luar, 2, ',', '.');
                                    } else {
                                        echo number_format($perkiraan_dlm, 2, ',', '.');
                                    }
                                    ?>
                                </td>
                            </tr>
                            <tr>
                                <td>Sisa Anggaran Sesudah Perkiraan</td>
                                <td>:</td>
                                <td>Rp. <?php
                                    if ($id_ref_tujuan == 1 or $id_ref_tujuan == 2 or $id_ref_tujuan == 3) {
                                        echo number_format($sisa_luar, 2, ',', '.');
                                    } else {
                                        echo number_format($sisa_dlm, 2, ',', '.');
                                    }
                                    ?></td>
                            </tr>
                        </table>
            ...
            <div class="modal-content">
                <form id="form_tambah" method="POST" action="<?= base_url() ?>index.php/Surat/Surat_masuk_keluar/aksi" enctype="multipart/form-data">
                    ...
                    <div class="modal-footer bg-blue">
                        ...
                        <button type="submit"  id="kembali" class="btn btn-danger" name="hasil" value='kembali'>Kembalikan</button>
                    </div>
                </form>
            </div>
        </div>
