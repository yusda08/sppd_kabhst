...
//        $this->API = base_url() . "reset_api/index.php";
    ...
    function index() {
        if ($this->session->userdata('is_login')) {
            ...
            $data['content'] = $this->load->view('admin/data/surat/surat_tugas_skpd', $record, TRUE);
    ...
    function surat_tugas($id_skpd) {
        if ($this->session->userdata('is_login')) {
            ...
            $record['javasc'] = $this->load->view('js', NULL, TRUE);
            $data['content'] = $this->load->view('admin/data/surat/surat_tugas', $record, TRUE);
            $this->load->view('layout', $data);
        }
    }
    function surat_perjalanan_dinas($id_skpd) {
        if ($this->session->userdata('is_login')) {
            $a = $this->session->userdata('is_login');
            $data['head'] = $this->load->view('head', NULL, TRUE);
            $data['main_header'] = $this->load->view('main_header', NULL, TRUE);
            $data['nav'] = $this->load->view('nav', NULL, TRUE);
            $data['foot'] = $this->load->view('foot', NULL, TRUE);
            $record['page_name'] = 'Surat Perjalanan Dinas';
            $id_spt = $this->input->get('spt');
            $record['id_skpd'] = $id_skpd;
            $record['id_spt'] = $id_spt;            
            $record['get_suratPerjalananDinasWhereSt'] = $this->Data_surat_tugas->get_suratPerjalananDinasWhereSt($id_skpd, $id_spt);
            $record['javasc'] = $this->load->view('js', NULL, TRUE);
            $data['content'] = $this->load->view('admin/data/surat/surat_perjalanan_dinas', $record, TRUE);
            $this->load->view('layout', $data);
        }
    }
...
//            $record['get_alat_angkut'] = $this->Data_referensi->get_alat_angkut();
    ...
    function get_ttdSpt($id, $id_skpd, $id_nd) {
        if ($this->session->userdata('is_login')) {
            $a = $this->session->userdata('is_login');
            $record['get_ttdSuratTugas'] = $this->Data_surat_tugas->get_ttdSuratTugas($id, $id_skpd, $id_nd);
            $record['get_pegawai'] = json_decode($this->curl->simple_get($this->API . '/Api_pegawai'));
            $get_page = $this->load->view('admin/data/surat/aksi/get_ttdSpt', $record, true);
            echo $get_page;
        }
    }
    ...
    function update_ttdSpt(){
        $a = $this->session->userdata('is_login');
        $id_kewenangan = $this->input->post('id_kewenangan');
        $url = $this->input->post('url');
        $id_spt = $this->input->post('id_spt');
        $req['status_ttd_spt'] = 1;
        $req['date_ttd_spt'] = date("Y-m-d H:i:s");
        $query = $this->Data_aksi->update('id', $id_spt, 'tbl_surat_tugas', $req);
        if ($query > 0) {
            $this->session->set_flashdata('tipe', 'alert-success');
            $this->session->set_flashdata('msg', 'Data berhasil dirubah');
            $activity = 'Merubah Data Surat Tugas yang Bertanda Tangan : ' . $id_spt;
            $this->Data_aksi->aktivitas($activity);
            redirect($url);
        } else {
            $this->session->set_flashdata('tipe', 'alert-danger');
