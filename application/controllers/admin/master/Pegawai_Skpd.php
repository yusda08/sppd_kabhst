...
//        $this->API = base_url() . "reset_api/index.php";
    ...
    function pegawaiPerSkpd($kunker) {
        if ($this->session->userdata('is_login')) {
            $a = $this->session->userdata('is_login');
            ...
            $data['content'] = $this->load->view('admin/data/master/pegawaiperskpd', $record, TRUE);
            $this->load->view('layout', $data);
        }
    }
...
//            }
    ...
    function delete_pegawai() {
        if ($this->session->userdata('is_login')) {
            ...
            $query = $this->Data_aksi->delete_multi('tbl_pegawai', $data);
            if ($query > 0) {
                $this->session->set_flashdata('tipe', 'alert-success');
